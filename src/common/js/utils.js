...
export function getRandomColorByIndex(index) {
	let colorList = ["#DB6D60", "#FFB650", "#6D7DFF", "#CA9662", "#1C8337", "#009688", "#D76B32", "#45385E", "#B17586", "#00D6A2", "#609F9E", "#B8732F", "#40426E", "#49776F", "#2F4F2F", "#4C766F", "#8D2324", "#D29178", "#876162", "#865935"];
...
export function formatPrice(price, unit = 2) {
  let num = formatPriceNumber(price, unit);
  return '¥' + ' ' + thousandOfCharacters(num);
}
export function formatPriceNumber(price, unit = 2) {
  price = Number(price);
  if (isNaN(price)) {
    price = 0;
  }
...
export function thousandOfCharacters(num) {
  num = +num;
  let numStr = "";
  // debugger
...
export function sortChange(event, form, vm, key = 'Sort', all = true, min = -10000, max = 10000, unit = 2){
  ...
  if (!Number.isInteger(value)) {
    value = parseFloat(value.toFixed(unit))
  }
...
export function formatNumber(number, digit = 2) {
  if (isNull(number)) {
    return number
  }
...
export function handleSearchDateRange(dateList, startKey, endKey, vm, form) {
  if (dateList[0] && dateList[1]) {
    vm[form][startKey] = dateList[0]
    vm[form][endKey] = dateList[1]
  } else {
    delete vm[form][startKey]
    delete vm[form][endKey]
  }
}
